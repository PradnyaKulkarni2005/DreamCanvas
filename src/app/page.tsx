...
export default function Page() {
  ...
  const handleLogout = async () => {
    ...
    localStorage.removeItem('token'); // optional if you store it
    setIsLoggedIn(false);
    router.push('/login');
  };
      ...
      <div className="absolute top-4 right-6 z-10">
        {isLoggedIn ? (
          <button
            ...
            className="px-5 py-2 rounded-full bg-white text-black font-medium hover:bg-red-200 transition duration-300 shadow-md"
          >
            Logout
          </button>
        ) : (
          <Link
            href="/login"
            className="px-5 py-2 rounded-full bg-white text-black font-medium hover:bg-emerald-200 transition duration-300 shadow-md"
          >
            Login
          </Link>
        )}
